...
ARK_API_EXPORT mapping* getFunctionsMapping()
{
    ...
    mapping_add(map[15], "http:client:post", http_client_post);
    mapping_add(map[16], "http:client:put", http_client_put);
    mapping_add(map[17], "http:client:delete", http_client_delete);
    mapping_add(map[18], "http:client:setFollowLocation", http_client_set_follow_location);

    // Client
    mapping_add(map[19], "http:client:setConnectionTimeout", http_client_set_co_timeout);
    mapping_add(map[20], "http:client:setReadTimeout", http_client_set_read_timeout);
    mapping_add(map[21], "http:client:setWriteTimeout", http_client_set_write_timeout);
    mapping_add(map[22], "http:client:setBasicAuth", http_client_set_basic_auth);
    mapping_add(map[23], "http:client:setBearerTokenAuth", http_client_set_bearer_token_auth);
    mapping_add(map[24], "http:client:setKeepAlive", http_client_set_keep_alive);
    mapping_add(map[25], "http:client:setProxy", http_client_set_proxy);
    mapping_add(map[26], "http:client:setProxyBasicAuth", http_client_set_proxy_basic_auth);
    mapping_add(map[27], "http:client:setProxyBearerTokenAuth", http_client_set_proxy_bearer_token_auth);
